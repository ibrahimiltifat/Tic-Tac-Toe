...
#assign buttons to all the boxes
for x in range(3):
    for y in range(3):
...
#initializing the variables when a thread is started
def init(a, b):
...
def sendCoordinate(c):
    ...
    if turn:
        if label3["ticked"] is not True:
            ...
            client.send(("|cor" + str(c[0]) + "|" + str(c[1])).encode())#sending the coordinate after doing a turn
            turn = False
...
def columnCheck():
    ...
    for i in range(columns):
        if listOfLabels[i]["symbol"] == listOfLabels[i + columns]["symbol"] == listOfLabels[i + columns + columns]["symbol"]:
...
def communicate(sender, m):
    ...
    while 1:
        ...
        if rcvd is None:
            ...
            label_status["text"] = "Welcome " + my_info["name"] + ":) We will connect you when another player comes"#this is player 1
        elif rcvd == "w2":
            label_status["text"] = "Welcome " + my_info["name"] + ":) We matched you with another player."#this is player 2

        elif rcvd.startswith("rvl"):
            oppo_info["name"] = rcvd[3:-1] #recieving name and symbol from the server and assigning it
            ...
            if my_info["symbol"] == "O":
                ...
                didYouStart = False
                turn = False
        elif rcvd.startswith("|cor"): #exchanging coordinates
            ...
            result = checkProcess()
            #starting a new game if either one has won. continuing otherwise
